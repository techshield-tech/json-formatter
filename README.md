...
```bash
git clone https://github.com/techshield-tech/json-formatter.git
cd json-formatter
bun install
bun dev
```

Open the URL Vite prints — by default **http://localhost:5173/json-formatter/**
(note the `/json-formatter/` path, see [Base path](#base-path)).
...
| otherwise (default)     | `/json-formatter/` | GitHub Pages                |
...
1. Go to [vercel.com/new](https://vercel.com/new) and import the
   `techshield-tech/json-formatter` repository.
...
`bun run build` with the default `/json-formatter/` base, and publishes `dist/`.
...
<script>
  ...
  window.addEventListener('message', (event) => {
    ...
    if (data?.slug !== 'json-formatter') return;
...
| iframe → parent | `{ type: 'mmoall-tool:ready', slug: 'json-formatter' }`          | Posted once on mount (embed mode only) |
| iframe → parent | `{ type: 'mmoall-tool:height', slug: 'json-formatter', height }` | Posted whenever the document height changes (embed mode only) |
